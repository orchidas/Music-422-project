...
"""
Created on Wed Mar  7 19:39:21 2018
...
import codecNew    # module where the actual PAC coding functions reside(this module only specifies the PAC file format)
...
#                print("For debugging :", codingParams.win_state, np.size(decodedData), np.size(codingParams.overlapAndAdd[iCh]))            
            ...
            data[iCh] = np.concatenate( (data[iCh],np.add(codingParams.overlapAndAdd[iCh],decodedData[:olaSamples]) ) )  # data[iCh] is overlap-and-added data
            codingParams.overlapAndAdd[iCh] = decodedData[olaSamples:]  # save other half for next pass
            
        #print('Decoded block length :', np.size(decodedData))
        #print('Decoded data length :' , np.size(data))
    ...
    def WriteDataBlock(self,data, codingParams):
        ...
        for iBlk in range(numSubBlocks):
            ...
            print(np.shape(fullBlockData), 'Window state :' , codingParams.win_state)
    ...
    def Close(self,codingParams):
        ...
        if self.fp.mode == "wb":  # we are writing to the PACFile, must be encode
            ...
            for iCh in range(codingParams.nChannels): data.append(np.zeros(nMDCTLines, dtype=float))
    ...
    def Encode(self,data,codingParams):
        ...
        return codecNew.Encode(data,codingParams)
    ...
    def Decode(self,scaleFactor,bitAlloc,mantissa, overallScaleFactor,codingParams):
        ...
        return codecNew.Decode(scaleFactor,bitAlloc,mantissa, overallScaleFactor,codingParams)
...
if __name__=="__main__":
  ...
  #  for Direction in ("Encode"):
        ...
        curState = 0
        #keeps track of next window's state
        nextState = 0
        prevBlock = []
        ...
        while True:
            ...
            #test what is being read and written
            if curBlock:
                
                if(Direction == "Encode"):
                    readSignal = np.concatenate([readSignal,curBlock[0]])
                elif(Direction == "Decode"):
                    writeSignal = np.concatenate([writeSignal, curBlock[0]])
            ...
            if nextBlock and Direction == "Encode":
                ...
                #find transient for each channel of data
                 for iCh in range(codingParams.nChannels):
                     ...
                     newBlock = np.copy(nextBlock[iCh])
                 ...
                 #determine next window state
                 nextState = W.nextBuffer(is_transient)
                 if (nextState == 0 and curState == 0) or nextState == 1 :
                     codingParams.win_state = 0
                 elif nextState == 2 and curState == 1:
                     codingParams.win_state = 1
                 elif nextState == 3 or (nextState == 2 and curState == 2): 
                     codingParams.win_state = 2
                 elif nextState == 0 and curState == 3:
                     codingParams.win_state = 0
                 #in case of transient after transient
                 elif nextState == 1 and curState == 3:
                     codingParams.win_state = 1
                     #codingParams.win_state = 3
            ...
            #you must ensure that you encode the transient and not the block following it                
            if(codingParams.win_state == 2):
                outFile.WriteDataBlock(prevBlock, codingParams)
                codingParams.win_state = nextState
